...
export class Score {
    ...
    static filter(colorsToExcitedProportion, colorsToCam) {
        ...
        for (const [color, cam] of colorsToCam.entries()) {
            const proportion = colorsToExcitedProportion.get(color);
            if (cam.chroma >= Score.CUTOFF_CHROMA && utils.lstarFromInt(color) >= Score.CUTOFF_TONE && proportion >= Score.CUTOFF_EXCITED_PROPORTION) {
                filtered.push(color);
            }
        }
        return filtered;
